...
**Repository:** Git. **`origin`** (primary, active — as of 2026-08-10) → `https://github.com/RickyGhb/training-portal` (private). All work happens here going forward: `git push origin main`. The remote URL has a fine-grained personal access token embedded (local-only, in `.git/config`, never committed) scoped to just this repo, Contents+Workflows read/write, expiring 2026-09-09 — rotate it (regenerate token on GitHub, update the remote URL) before then or pushes will start failing with an auth error. `gitlab` remote (`ssh://git@gitlab.endeavourtech.io:33/ricky/training-portal.git`) is still configured and the GitLab project itself is untouched/preserved, but **no longer pushed to** — GitLab is retired from the active workflow by explicit user decision (2026-08-10), not a technical necessity.
...
9. ~~Wire up CI (lint/build on every push)~~ — done 2026-08-10 via GitHub Actions (`.github/workflows/ci.yml`), verified green (run #1, 1m22s). GitLab's own `.gitlab-ci.yml` (left in place on that project, unused now) never worked — its only registered runner (`#4`, `YGZDobYsd`) shows `Active: Yes` but last checked in 2 years ago, no live process anywhere. Rather than standing up runner infrastructure there, the project moved to GitHub entirely (see Repository above) — this doubled as the fix. E2E-in-CI is a further follow-on, needing staging DB + Upstash credentials added as GitHub Actions secrets.
...
**Last Updated:** 2026-08-10 — enterprise-scale hardening pass: added indexes on the offshore/placement `User` fields, wrapped the marketing-readiness status flip in a transaction, added security headers and top-level error boundaries, added login rate limiting (Upstash Redis), stood up a separate staging Supabase project so local dev no longer writes into the production database, added Sentry error monitoring, and migrated the repo's active remote from GitLab to a private GitHub repo (`origin`) — GitLab's pipeline never had a live runner; GitHub Actions does, verified green. By explicit user decision the GitLab project is left in place untouched but is no longer pushed to. Restricted report-export access to CEO/Location Manager only (previously included Location Admin). See "Repository", "Staging vs. production", and the Sentry/rate-limiting notes under Tech Stack and User Management for details.
