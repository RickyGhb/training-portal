...
**Last Updated:** 2026-08-10 — enterprise-scale hardening pass: added indexes on the offshore/placement `User` fields, wrapped the marketing-readiness status flip in a transaction, added security headers and top-level error boundaries, added login rate limiting (Upstash Redis), stood up a separate staging Supabase project so local dev no longer writes into the production database, added `.gitlab-ci.yml` (lint+build — pipeline definition is real but currently has no live runner to execute it, see Next Steps), and added Sentry error monitoring. Restricted report-export access to CEO/Location Manager only (previously included Location Admin). See "Staging vs. production" and the Sentry/rate-limiting notes under Tech Stack and User Management for details.
