...
│   │       └── [id]/edit/, [id]/submissions/  # Builder UI; responses spreadsheet grid (sortable, searchable, CSV/XLSX export, paginated 50/page)
...
│       ├── forms/export/[id]/route.ts          # CSV/XLSX download of a form's responses — CSRF-gated, same scoping as the responses page. Path order is export/[id], not [id]/export — see "Forms" under Key Features
...
│   ├── formResponses.ts              # loadFormForViewer/buildSubmissionWhere/getFormResponseRows/formatAnswerValue — shared by the responses grid and its export, so the two can't drift on scoping
...
### 6. Audit Logging
- 35 `AuditActionType` values, every sensitive mutation across the app writes one via `logAudit()` (or, for login events, a direct `prisma.auditLog.create` call in `src/app/login/actions.ts`, which intentionally bypasses `logAudit()`)
...
- **Viewing responses** (`/forms/[id]/submissions`, rebuilt 2026-08-19): a **spreadsheet grid** — one row per response, one column per question in `FormField.sortOrder`, every answer visible at once, no per-row expanding. Wide forms scroll sideways inside the table's own container with the `Submitted` column pinned left (`.grid-shell`/`.grid-table`/`.grid-sticky` in `globals.css` — deliberately *not* `.table-shell`, whose `overflow: hidden` would silently break `position: sticky`). Answers are keyed by `fieldId`, so a question the respondent skipped (no `FormAnswer` row at all) renders an empty cell instead of shifting every later column. Long answers clip at 80 chars with the full text on hover; `FILE_UPLOAD` cells render the authenticated `/api/forms/files/[fileId]` links inline. Column headers are plain sort links (`?sort={fieldId}&dir=`), and a GET filter bar offers a text search across answers plus a submitted-between date range — all filter/sort state lives in the URL. Clicking a row expands it (client state in `submissions/response-row.tsx`) into a full-width panel underneath listing every answer untruncated plus that response's files — the grid stays scannable while long paragraph answers remain readable in place; the Delete button stops click propagation so it can't double as a toggle, and the `Submitted` label is preformatted on the server to avoid a locale/timezone hydration mismatch. Paginated 50/page. Everything the page and the export share lives in **`src/lib/formResponses.ts`**: `loadFormForViewer()` (authorization, returning `"not-found"` / `"forbidden"` / the form + its fields), `buildSubmissionWhere()` (viewer scope **AND**-ed with the user's filters, never merged — same discipline as `consultantScopeFilter` in `reports.ts`), `getFormResponseRows()`, and the pure `formatAnswerValue()`. Sorting by a *question* column can't be a Prisma `orderBy` (the value lives in a related row), so that path resolves the page's ids from a narrow query first and only then pulls the full answer/file include for those 50 rows. Text search covers both answer shapes: `valueText` via a Prisma `contains`, and CHECKBOXES answers (which live in a `valueJson` JSONB array, where Prisma's JSON filters only offer exact element matching) via `findCheckboxAnswerMatches()` — one parameterized `$queryRaw` restricted to `CHECKBOXES` fields and guarded by `jsonb_typeof(...) = 'array'`, whose matching submission ids are OR'd into the same `AND` branch as the text match, so a hit can never escape the viewer's scope. Known quirk: a search consisting of a literal `%` or `_` matches everything, because Prisma's `contains` doesn't escape LIKE wildcards (the raw checkbox query does escape them). Harmless — scoping still applies — but it is why the two halves can disagree on such a search.
- **Exporting responses**: `GET /api/forms/export/[id]?format=csv|xlsx` (added 2026-08-19), reusing `csvEscape` and ExcelJS exactly as `/api/reports/export` does, and CSRF-protected by the same `src/lib/csrf.ts` synchronizer token for the same reason (a plain GET Route Handler gets no automatic Server Action CSRF protection). It runs through the identical `loadFormForViewer`/`buildSubmissionWhere` pair as the page, so a location-scoped viewer's download can never contain rows their screen doesn't show, and it carries the page's current filters. Note the route lives at `/api/forms/export/[id]`, **not** `/api/forms/[id]/export` — a sibling dynamic segment named `[id]` alongside the existing `[slug]` is a Next.js build error ("cannot use different slug names for the same dynamic path"); the static `export/` segment sits beside `files/` instead, the same way that one already does.
- **Deleting a response**: `deleteSubmissionAction` (`forms/actions.ts`), gated through the existing `requireFormEditor` — the form's creator or CEO only, deliberately **not** location-scoped or grant-holding viewers, who may read responses but never destroy them. Re-checks the `submissionId` against the `formId` (the same cross-form IDOR discipline as the 2026-08-15 field/grant fixes), deletes the submission's Vercel Blob files *before* the row (the `FormFileUpload` rows cascade away with it, so doing it after would orphan the blobs), and logs `FORM_SUBMISSION_DELETED`. The button is a thin client wrapper, `submissions/delete-submission-button.tsx` — see the Server Action registration note under Code Quality & Standards for why that wrapper is load-bearing rather than stylistic.
...
**Unit tests** (`npm run test:unit`, Vitest, `vitest.config.mts`): 277 tests across 16 files in
...
- Other pure files with dedicated tests: `drive.ts`, `validation/user.ts`, `validation/catalog.ts`,
  ...
  `formResponses.ts` (added 2026-08-19 — `formatAnswerValue` across every field type including
  the skipped-question and empty-checkbox cases, plus `buildSubmissionWhere` asserting the
  location scope survives every filter combination),
...
- `prisma/migrations/20260815221001_add_performance_indexes/` — adds 6 composite indexes found missing during a 2026-08-15 performance review: `User[role,deletedAt]`, `AuditLog[actionType,createdAt]`, `FormSubmission[formId,submittedAt]`, `FormAccessGrant[grantedToUserId]`, `Session[userId,revokedAt]`, `Form[createdByUserId]`. Applied to staging 2026-08-15 and to production 2026-08-16.
- `prisma/migrations/20260819172000_add_form_submission_deleted_audit_action/` — adds the `FORM_SUBMISSION_DELETED` `AuditActionType` value. Applied to staging 2026-08-19; **not yet applied to production** — run it before/alongside the next deploy or `deleteSubmissionAction` will fail there on its `logAudit` write.
...
- **Server Action registration (learned the hard way 2026-08-19):** a Server Action is only registered for a route if some **client** component in that route's own module graph imports it. Handing an action straight from a Server Component to a client component like `ConfirmButton` works *only* when a sibling client component on the same route already imports that actions module — which is why `locations/page.tsx` can pass `setLocationStatusAction` directly (its sibling `location-form.tsx` is `"use client"` and imports `./actions`), while doing the same on `/forms/[id]/submissions` failed: no client component on that route imported `forms/actions.ts` at all. The symptom is a runtime `Failed to find Server Action "<id>"` and a "Something went wrong" boundary on submit, in production builds as well as dev. Verified by A/B'ing both shapes on clean rebuilds; the import path (`@/` alias vs relative) makes no difference. The reliable pattern — and what every other row action in the app already does — is a small `"use client"` wrapper that imports the action itself.
...
- **`20260819172000_add_form_submission_deleted_audit_action` is on staging but not yet on production** — the response-delete action's audit write will fail there until it's applied.
- ~~Six new composite indexes are on staging but not yet on production~~ — **applied to production 2026-08-16**; re-verified 2026-08-19 with `prisma migrate status` against the production database, which reports every migration through `20260815221001_add_performance_indexes` as applied.
...
- `docs/Video-Hosting-Decision.md` (added 2026-08-16) — why training videos are moving off Google Drive, what content protection is and isn't achievable, a five-vendor cost comparison at ~800k delivered min/month, and the migration plan. **Platform choice still open** (Cloudflare Stream was selected, then disqualified on discovering it has no DRM; Mux + DRM is the current recommendation at roughly the same cost). Nothing in it is built yet.
