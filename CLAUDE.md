...
**Last Updated:** 2026-08-10 — enterprise-scale hardening pass: added indexes on the offshore/placement `User` fields, wrapped the marketing-readiness status flip in a transaction, added security headers and top-level error boundaries, added login rate limiting (Upstash Redis), stood up a separate staging Supabase project so local dev no longer writes into the production database, added Sentry error monitoring, and fully migrated the repo from GitLab to a private GitHub repo (`origin`) — GitLab's pipeline never had a live runner; GitHub Actions does, verified green. The GitLab project was ultimately deleted outright by explicit user decision (not just abandoned). Restricted report-export access to CEO/Location Manager only (previously included Location Admin). See "Repository", "Staging vs. production", and the Sentry/rate-limiting notes under Tech Stack and User Management for details.
