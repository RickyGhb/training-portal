...
**Repository:** Git, two remotes. `gitlab` (primary, original) → `ssh://git@gitlab.endeavourtech.io:33/ricky/training-portal.git`. `github` (added 2026-08-10, mirror — pushed to for CI only, not the primary) → `https://github.com/RickyGhb/training-portal` (private). Push both when making changes: `git push gitlab main && git push github main`. The `github` remote's URL has a fine-grained personal access token embedded (local-only, in `.git/config`, never committed) scoped to just this repo, Contents+Workflows read/write, expiring 2026-09-09 — rotate it (regenerate token on GitHub, update the remote URL) before then or pushes to `github` will start failing with an auth error.
...
### Deployment
- Hosting: Vercel, project `ricky-s-team1/training-portal`. **No GitHub/GitLab webhook integration** — deploys are manual: `npx vercel --prod` from this folder. Pushing to either `gitlab/main` or `github/main` does NOT redeploy the live app by itself, and CI passing/failing on either does not gate deploys — this is deliberate scope, not wired up.
...
9. ~~Wire up CI (lint/build on every push)~~ — done 2026-08-10, but not on GitLab. `.gitlab-ci.yml` still exists and is left in place, but pipelines there sit permanently `Pending`: this GitLab instance (`gitlab.evr-tec.com`)'s only registered project runner (`#4`, `YGZDobYsd`) shows `Active: Yes` but last checked in 2 years ago — no live `gitlab-runner` process anywhere. Rather than standing up runner infrastructure, the repo was also mirrored to GitHub (`github` remote, see Repository above) with `.github/workflows/ci.yml` running the identical lint+build job on GitHub-hosted runners — verified green (run #1, 1m22s) same day. GitLab remains the primary remote; GitHub exists solely so CI actually executes. E2E-in-CI is a further follow-on beyond either, needing staging DB + Upstash credentials added as CI/CD variables/secrets on whichever platform runs it.
...
**Last Updated:** 2026-08-10 — enterprise-scale hardening pass: added indexes on the offshore/placement `User` fields, wrapped the marketing-readiness status flip in a transaction, added security headers and top-level error boundaries, added login rate limiting (Upstash Redis), stood up a separate staging Supabase project so local dev no longer writes into the production database, added Sentry error monitoring, and got CI actually running — GitLab's own pipeline (`.gitlab-ci.yml`) has no live runner, so the repo was also mirrored to a private GitHub repo (`github` remote) purely for GitHub Actions' free hosted runners; GitLab stays primary. Restricted report-export access to CEO/Location Manager only (previously included Location Admin). See "Repository", "Staging vs. production", and the Sentry/rate-limiting notes under Tech Stack and User Management for details.
